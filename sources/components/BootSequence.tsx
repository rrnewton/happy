...
const BOOT_LINES = [
    ...
    { text: '', type: 'empty' },
    { text: 'POST: Power-On Self Test', type: 'dim' },
    { text: 'CPU: Neural Processing Unit @ 3.2 THz... OK', type: 'info' },
    { text: 'Memory: 128 PB Quantum RAM... OK', type: 'info' },
    { text: 'Storage: 1 EB Holographic Array... OK', type: 'info' },
    { text: 'GPU: Photonic Rendering Engine x4... OK', type: 'info' },
    { text: '', type: 'empty' },
    { text: '0x7FFF5FBFF8A0 Kernel space allocated', type: 'hex' },
    { text: '0x00007F8B2C000000 User space mapped', type: 'hex' },
    { text: '', type: 'empty' },
    { text: 'Initializing quantum kernel...', type: 'dim' },
    { text: 'Loading neural drivers...', type: 'dim' },
    ...
    { text: '[  OK  ] Started Holographic Display Driver', type: 'success' },
    ...
    { text: '', type: 'empty' },
    { text: 'BIOMETRIC SCAN... VERIFIED', type: 'scan' },
    { text: 'NEURAL LINK... ESTABLISHED', type: 'scan' },
    { text: 'IP: 10.42.0.1 via quantum-tunnel-0', type: 'network' },
    { text: 'SECURE TUNNEL TO HAPPY.ENGINEERING... OK', type: 'network' },
    { text: '', type: 'empty' },
    { text: 'All systems nominal. Welcome back.', type: 'info' },
];
...
export const BootSequence = React.memo(function BootSequence({ onComplete }: BootSequenceProps) {
    ...
    React.useEffect(() => {
        ...
        // Start progress bar
        const totalDuration = BOOT_LINES.length * 60 + 600; // Faster timing
        ...
        const showNextLine = () => {
            if (currentLine < BOOT_LINES.length) {
                ...
                // Variable delay based on line type - faster timing
                const line = BOOT_LINES[currentLine - 1];
                let delay = 50; // Base delay (faster)

                if (line.type === 'empty') {
                    delay = 25; // Empty lines are quick
                } else if (line.type === 'header') {
                    delay = 80; // Headers slightly slower
                } else if (line.type === 'success') {
                    delay = 40; // Service starts are fast
                } else if (line.type === 'dim') {
                    delay = 100; // "Loading..." lines pause longer
        ...
        const startTimer = setTimeout(showNextLine, 300);
    ...
    React.useEffect(() => {
        if (phase === 'complete') {
            ...
            accessGrantedOpacity.value = withDelay(150, withTiming(1, { duration: 300 }));
            accessGrantedScale.value = withDelay(150, withSequence(
