...
export const zhHans: TranslationStructure = {
    ...
    settings: {
        ...
        codingStats: '编码统计',
        codingStatsSubtitle: '查看来自 Wakapi 的编码活动',
        wakapi: 'Wakapi 设置',
        wakapiSubtitle: '配置 Wakapi 集成',
    ...
    settingsVoice: {
        ...
        customApiUrlHint: '使用您自己的 Whisper 兼容 API URL 替代默认的 OpenAI API 端点。留空则使用 OpenAI。',
        customModel: '自定义模型',
        customModelPlaceholder: 'whisper-1',
        customModelHint: '指定使用哪个 Whisper 模型。常用选项：whisper-1（默认）、whisper-large-v3 或其他兼容模型。留空则使用默认模型。',
    ...
    settingsEnvironments: {
        ...
        helpText: '点击星标以设为默认应用。可以将多个集合标记为默认。点击环境变量集可编辑或删除。',
        tipText: '环境变量集在启动新会话时应用。可以选择多个集合并合并使用（后面的集合会覆盖前面的）。可以使用 ANTHROPIC_BASE_URL 和 ANTHROPIC_AUTH_TOKEN 等变量连接到不同的 API 提供商。',
    },
    ...
    settingsWakapi: {
        status: '状态',
        statusDescription: '使用 Wakapi 或 WakaTime 跟踪您的编码活动',
        codingStats: '编码统计',
        notConfigured: '未配置',
        configured: '已配置',
        connectedAs: ({ username }: { username: string }) => `已连接为 ${username}`,
        setupPrompt: '连接 Wakapi 以查看您的编码统计',
        configure: '配置',
        apiConfiguration: 'API 配置',
        apiConfigurationDescription: '输入您的 Wakapi 或 WakaTime API 凭据',
        apiUrl: 'API URL',
        whatIsWakapi: '什么是 Wakapi？',
        apiKey: 'API 密钥',
        apiKeyPlaceholder: '您的 API 密钥',
        apiKeyHint: '在 Wakapi 设置中的 API > API 密钥 下找到您的 API 密钥',
        corsProxy: 'CORS 代理',
        corsProxyDescription: '网页浏览器访问 Wakapi API 时需要。部署代理工作器并在此处输入其 URL。',
        useProxy: '使用 CORS 代理',
        useProxySubtitle: '启用代理以供网页浏览器访问',
        proxyUrl: '代理 URL',
        testConnection: '测试连接',
        save: '保存',
        connectionSuccess: ({ username }: { username: string }) => `已成功连接为 ${username}`,
        apiUrlAndKeyRequired: '需要 API URL 和 API 密钥',
        configureFirst: '请先配置 API URL 和 API 密钥',
        settingsSaved: 'Wakapi 设置已保存',
        totalTime: '总时间',
        dailyAverage: '日均时间',
        projects: '项目',
        languages: '语言',
